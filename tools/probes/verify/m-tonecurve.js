/**
 * (m2) TONE CURVE, ROUND 8 — the delivered display response, measured, not
 * modelled. A copy of k-tonecurve.js with the ablation re-pointed: `before` is
 * setPrint('r7') and `after` is setPrint('r8'), i.e. the WHOLE previous print
 * against the whole new one (the real Fox Engine curve, the shadow expansion
 * below its knee, the removed `lift` pedestal, the re-gated shadow fill).
 * Those move together — they are all statements about the same band — so
 * ablating any one of them alone measures a fraction of a change.
 *
 * THE IDENTITY THIS ROUND IS GOVERNED BY. For a print whose log-log slope is
 * q at display code C,
 *
 *     codes per stop = 0.2887 * q * (C + 14)
 *
 * (differentiate 255*sRGB(y) with respect to log2 y). Two consequences worth
 * writing down, because they decide what is and is not reachable:
 *
 *  - Codes per stop is very nearly INVARIANT to q at a fixed scene radiance.
 *    Raising q lowers C by almost exactly as much as it raises the multiplier:
 *    at the measurement point below, q = 0.9 / 1.05 / 1.2 all deliver 7.5-7.6.
 *    You cannot buy shadow gradient with a steeper shadow slope.
 *  - Therefore the only lever is C itself, i.e. a pedestal. "12 codes/stop at
 *    scene-linear 0.010" needs C ~ 27 there; "p0.1 = 18.4" (MGSV's measured
 *    value) caps it at 9.4. The two halves of that requirement are arithmetic
 *    opposites, and the real game satisfies the second one, not the first.
 *
...
// The harness no longer forwards argv to a probe; edit this to re-point it.
const shot = 'gameplay';
g.applyShot(shot);
g.settle(6);
