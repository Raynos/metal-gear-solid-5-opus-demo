...
/**
 * THE ONE THING THAT KILLED THE LAST DAEMON.
 *
 * Whichever checkout ran a client first became the daemon's CODE. Six of seven
 * worktrees carried stale copies, so a concurrency fix, a cap fix and the whole
 * metrics layer were silently not running for hours — the daemon was worktree-5's
 * build and nothing said so. It was only found by printing the daemon's own argv.
 *
 * So: the daemon is pinned to ONE tree, it publishes the hash of its own source,
 * and a client whose source differs is told rather than served stale behaviour.
 */
const SELF_HASH = (() => {
  try {
    return createHash('sha1').update(readFileSync(new URL(import.meta.url))).digest('hex').slice(0, 12);
  } catch {
    return 'unknown';
  }
})();
const HOME_TREE = path.resolve(path.dirname(new URL(import.meta.url).pathname), '..');
...
// Idle exit is what makes a singleton safe to leave running: it costs nothing
// when nobody is using it, and a client spawns it again on demand. The previous
// version never actually exited under load because lastActivity was touched by
// bookkeeping as well as by work.
...
// Cap by MEASURED resident memory, not a guessed count. Guessing went 3 (worlds
// evicted each other continuously, 138 rebuilds in a session), then 10 (six
// resident worlds cost 10.5 GB), then 5 — all blind. A world is ~1.7 GB once GPU
// resources count, which the JS heap's 0.15 GB does not show. The count is a
// backstop; the budget is the real limit.
const MAX_WORLDS = Math.max(1, Math.min(+opt('worlds', 12), 16));
const MEM_BUDGET_MB = Math.max(1024, +opt('mem', 6144));

function residentMB() {
  try {
    const out = execSync('ps -A -o rss=,args=', { encoding: 'utf8' });
    let kb = 0;
    for (const line of out.split('\n')) {
      if (/chrome-headless-shell|vite\/bin\/vite\.js/.test(line)) kb += parseInt(line, 10) || 0;
    }
...
async function evictIfNeeded(keepRoot) {
  // Evict while EITHER limit is exceeded. Memory is the real constraint and the
  // count is a backstop for the case where ps is unavailable.
  while (worlds.size >= MAX_WORLDS || (worlds.size > 1 && residentMB() > MEM_BUDGET_MB)) {
    let oldest = null;
