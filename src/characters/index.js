...
export async function install(world) {
  ...
  // --- per-frame ----------------------------------------------------------
  //
  // LOD bands, in metres from the camera. See `Character.LOD_STEP` for why the
  // update intervals are what they are; these are the distances at which a
  // skipped frame stops being worth a pixel.
  //
  // `lodBias` exists so the measurement can be ablated in place: setting it to
  // a huge number forces every character to LOD 0 with no rebuild, which is the
  // only honest way to price this — a per-pass flag that gates nothing has been
  // this project's recurring measurement failure.
  const LOD_DIST = [11, 30, 62];
  const stats = { chars: 0, lod: [0, 0, 0, 0], ms: 0, msAvg: 0, frames: 0 };
  const api = { lodEnabled: true, stats };
  ...
  engine.addSystem({
    ...
    update(dt) {
      ...
      stats.lod[0] = stats.lod[1] = stats.lod[2] = stats.lod[3] = 0;
      for (const ch of characters) {
        // The player is never LODed: he is the subject of every frame and he is
        // the one character a stepped gait would be visible on.
        let lod = 0;
        if (api.lodEnabled && !ch.isPlayer && cam) {
          const d = Math.hypot(ch.position.x - _cam.x, ch.position.z - _cam.z);
          lod = d > LOD_DIST[2] ? 3 : d > LOD_DIST[1] ? 2 : d > LOD_DIST[0] ? 1 : 0;
        }
        ch.lod = lod;
        // "No IK beyond a few metres" in the sense that matters: the four-tap
        // terrain normals behind the body tilt and the sole conform go away.
        // The limb IK itself stays on at every distance, because without it the
        // legs have no pose at all — they are solved, not keyframed.
        ch.anim.coarse = lod >= 2;
        stats.lod[lod]++;
      ...
      stats.frames++;
      // Exponential mean over ~1 s at 60 Hz. A single frame's reading on this
      // system is dominated by whether a GC landed in it.
      stats.msAvg = stats.frames < 2 ? stats.ms : stats.msAvg * 0.98 + stats.ms * 0.02;
    },
  });
  ...
  return {
    ...
    commander,
    /** Animation LOD: live counters, and a switch so it can be ablated. */
    lod: api,
