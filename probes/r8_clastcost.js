/**
 * r8_clastcost.js — what the round-8 ground cover actually costs, measured as an
...
const arms = {
  on: { set: () => { for (const m of clast) m.visible = true; }, ms: [] },
  off: { set: () => { for (const m of clast) m.visible = false; }, ms: [] },
};
...
return {
  ...
  msWithClast: +med(arms.on.ms).toFixed(2),
  msWithoutClast: +med(arms.off.ms).toFixed(2),
  clastCostMs: +(med(arms.on.ms) - med(arms.off.ms)).toFixed(2),
  samplesOn: arms.on.ms.map((v) => +v.toFixed(1)),
  samplesOff: arms.off.ms.map((v) => +v.toFixed(1)),
  drawsWithClast: stats.on.calls,
  drawsWithoutClast: stats.off.calls,
  trisWithClast: stats.on.triangles,
  trisWithoutClast: stats.off.triangles,
